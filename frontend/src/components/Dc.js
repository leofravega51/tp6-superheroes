...
export default function Dc() {
    ...
    const [dcHeros, setDcHeros] = useState([]);
    const history = useHistory();

    const getHeros = async () => {

        const response = await axios.get('http://localhost:5000/dc');
        setDcHeros(response.data)
    }
                ...
                {dcHeros.map((h,i) => (
                    <div className="card" style={{width: "18rem", margin: "2% 3%", minHeight: "20rem"}} key={i}>
                        ...
                        <div className="card-body">
                            ...
                            <a className="btn btn-primary" onClick={() => handleInformation(h.name)} style={{color: "white", marginLeft: "10%"}}>More info</a>
                            <a className="btn btn-danger" onClick={() => handleDelete(h.name)} style={{color: "white", marginLeft: "10%"}}>Delete</a>
                        </div>
                    </div>
                ))}
                </div>
